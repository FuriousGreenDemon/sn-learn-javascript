/*
Create an array of 4 strings, and then set a custom property for that array. The goal is to have it display each string on separate line using a loop. Run a `forEach` loop on that array, a three expressions `for` loop on the same array, and then a `for...in` array on that array to make sure you see the key difference. What is the difference on the last loop syntax? Let me know in the comments which way you prefer now that you know three different ways to do it.
*/
